...
final class ReceivingConsumer<K, V> implements ConsumerRebalanceListener, ConsumerInvocable {
    ...
    private Object invokeConsumerFromExternal(Method method, Object[] args) throws ReflectiveOperationException {
        ...
        if (method.getName().equals("pause")) {
            Collection<TopicPartition> partitions = (Collection<TopicPartition>) args[0];
            handleExternalPauseResume(Consumer::pause, PartitionListener::onPartitionsExternallyPaused, partitions);
            return null;
        } else if (method.getName().equals("resume")) {
            Collection<TopicPartition> partitions = (Collection<TopicPartition>) args[0];
            handleExternalPauseResume(Consumer::resume, PartitionListener::onPartitionsExternallyResumed, partitions);
            return null;
        } else {
    ...
    private void handleExternalPauseResume(
        BiConsumer<Consumer<?, ?>, Collection<TopicPartition>> consumerInvocation,
        BiConsumer<PartitionListener, Collection<TopicPartition>> partitionListenerInvocation,
        Collection<TopicPartition> partitions
    ) {
        try {
            consumerInvocation.accept(consumer, partitions);
        } catch (IllegalStateException e) {
            LOGGER.debug("Consumer not actively assigned partitions, but pausing/resuming anyway");
        }
        partitionListenerInvocation.accept(partitionListener, partitions);
    }
