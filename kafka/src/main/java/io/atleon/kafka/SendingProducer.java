...
final class SendingProducer<K, V> implements ProducerInvocable {
    ...
    public void sendSafely(ProducerRecord<K, V> producerRecord, Supplier<Boolean> producePredicate, Callback callback) {
        if (sendImmediate) {
    ...
    private void send(ProducerRecord<K, V> producerRecord, Supplier<Boolean> producePredicate, Callback callback) {
        try {
            if (producePredicate.get()) {
