...
final class ProhibitableConsumerFactory extends ConsumerFactory {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(ProhibitableConsumerFactory.class);

    private volatile Instant invocationDeadline = null;
    ...
    public void prohibitFurtherInvocations(Duration gracePeriod) {
        invocationDeadline = Instant.now().plus(gracePeriod);
    }
    ...
    private final class ConsumerProxy<K, V> implements Consumer<K, V> {
        ...
        public ConsumerProxy(ReceiverOptions<K, V> config) {
            this.clientId = config.clientId();
            this.delegate =
                new KafkaConsumer<>(config.consumerProperties(), config.keyDeserializer(), config.valueDeserializer());
        }
        ...
        public ConsumerRecords<K, V> poll(Duration timeout) {
            return invokeIfAllowed("poll", () -> delegate.poll(timeout));
        }
        ...
        private <T> T invokeIfAllowed(String name, Supplier<T> supplier) {
            Instant deadline = invocationDeadline;
            if (deadline == null || Instant.now().compareTo(deadline) < 0) {
                return supplier.get();
            } else {
                RuntimeException exception =
                    new IllegalStateException("Consumer invocations prohibited, but " + name + " invoked: " + clientId);
