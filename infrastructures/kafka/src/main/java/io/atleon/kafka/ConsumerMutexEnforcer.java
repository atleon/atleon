 ...
 * any given instant. This helps to work around an observed bug in Reactor Core where downstream
 * cancellations may not be fully propagated upstream, which makes it possible for a Consumer and
 * its polling loop to become orphaned. In the presence of paused assigned partitions on that
 * Consumer instance, this can cause the orphaned Consumer to silently occupy assigned partitions
 * without emitting anything for processing.
 */
class ConsumerMutexEnforcer {
    ...
    public final <K, V> Function<Map<String, Object>, Consumer<K, V>> newConsumerFactory() {
    ...
    private interface Invoker {
        ...
        <T> T invokeIfAllowed(Supplier<T> supplier);
    }

    private final class TicketedInvoker implements Invoker {

        private final int ticket;

        private TicketedInvoker(int ticket) {
            this.ticket = ticket;
        }

        @Override
        public <T> T invokeIfAllowed(Supplier<T> supplier) {
            if (ticket != ticketCounter.get()) {
                RuntimeException exception = new IllegalStateException("Detected orphaned Consumer instance!");
    ...
    private static final class ConsumerProxy<K, V> implements Consumer<K, V> {

        private final Consumer<K, V> delegate;

        private final Invoker invoker;

        public ConsumerProxy(Consumer<K, V> delegate, Invoker invoker) {
        ...
        public ConsumerRecords<K, V> poll(Duration timeout) {
            return invoker.invokeIfAllowed(() -> delegate.poll(timeout));
        }
