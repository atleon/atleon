...
final class SendPublisher<T> implements Publisher<RabbitMQSenderResult<T>> {
    ...
    private abstract static class Send<T>
        ...
        public void onNext(RabbitMQSenderMessage<T> message) {
            ...
            Mono<Void> publish = channel.publish(runner, deliveryTag -> {
                ...
                inFlightPublishes.put(deliveryTag, publishArguments.toOutboundDelivery(correlationMetadata));
        ...
        private void enqueueDetachment(Consumer<Subscriber<?>> andThen) {
            emissionQueue.addAndDrain(subscriber -> {
                runSafely(() -> channel.removeConfirmListener(this), "channel::removeConfirmListener");
                runSafely(() -> channel.removeReturnListener(this), "channel::removeReturnListener");
                runSafely(() -> channel.removeShutdownListener(this), "channel::removeShutdownListener");
