...
public final class TracingAloReceivedRabbitMQMessageDecorator<T>
    ...
    @Override
    protected SpanBuilder newSpanBuilder(SpanBuilderFactory spanBuilderFactory, ReceivedRabbitMQMessage<T> message) {
        return spanBuilderFactory
                .newSpanBuilder("atleon.receive.rabbitmq")
                .setAttribute("queue", Objects.toString(queue))
                .setAttribute("exchange", message.exchange())
                .setAttribute("routing_key", message.routingKey());
    }
