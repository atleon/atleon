...
### Additions
* [#589] Added `AloStream::newVirtualThreadScheduler` convenience method for using virtual threads in `AloStream` implementations using JDK 21+

### Updates
* [#586] Replaced dependence on Bucket4j for throughput limiting with simple zero-dependence smooth token-bucketing algorithm

### Removals
* [#588] Removed various old deprecated methods+constants with least intrusiveness

# [0.46.0]
atleon `0.46.0` is a beta release containing updates outlined below

### Additions
* [#577] Micrometer Observation: Added `micrometer-observation` module to facilitate observability integration for Kafka, and further support migration from `reactor-kafka`

# [0.45.0]
atleon `0.45.0` is a beta release containing updates outlined below

### Additions
* [#574] Core: Added convenient `AloFlux::decorateAlo` operator to support `AloDecorator` (re)usage

### Updates
* [#568] Baseline JDK version bumped from 1.8 to 17

# [0.44.0]
atleon `0.44.0` is a beta release containing updates outlined below

### Additions
* [#558] Core: Support `EAGER` upstream demand from `AcknowledgementQueue`
* [#564] Core/Kafka: Added convenience methods for switching to/from high-level `Alo` API from low-level `Flux`/clients

### Fixes
* [#562] Kafka: Guard against erroneous negative ack followed by positive ack tracking

# [0.43.0]
atleon `0.43.0` is a beta release containing updates outlined below

### Additions
* [#544] Kafka: Enable durable "acknowledged-ahead-of-commit" offset tracking via `OffsetTrackingStrategy` 

# [0.42.0]
atleon `0.42.0` is a beta release containing updates outlined below

### Additions
* [#539] Core: Added `AloFlux::groupByWithAutoComplete` operator supporting unbounded grouping cardinality with automatic group completion

# [0.41.1]
atleon `0.41.1` is a beta release containing updates outlined below

### Fixes
* [#529] Core: (Micro)optimize away unnecessary arithmetic in Murmur3
...
### Fixes
* [#450] Replace usage of Jetbrains Annotations with JSpecify (and NullAway)
* [#353] Integrate with Spotless (and Palantir Java Format) and apply

### Removals
* [#468] Eliminated dependence on Reactor Kafka

# [0.36.2]
atleon `0.36.2` is a beta release containing updates outlined below

### Fixes
* [#460] kafka: Detect+Ensure consumer mutex on (`Alo`-specific) reception methods

# [0.36.1]
atleon `0.36.1` is a beta release containing updates outlined below

### Fixes
* [#456] kafka: Ensure sender request to upstream is aggregately capped at `maxInFlight`

# [0.36.0]
atleon `0.36.0` is a beta release containing updates outlined below

### Additions
* [#451] Added typed JSON deserializers

### Fixes
* [#448] kafka: Enforcing timeout on finite reception via revocation grace period
* [#452] kafka: Ensuring idempotency is explicitly set (to `true`) by default
...
### Fixes
* [#352] Removed transitive JUnit dependency from zookeeper-embedded
* [#439] Kafka: Increase logging visibility on revocation/closure commit timing

# [0.35.0]
atleon `0.35.0` is a beta release containing updates outlined below

### Fixes
* [#385] Kafka: Use optimized clients by default (now supporting Kafka >=4.x)
* [#427] Kafka: Added missing factory methods for Priority Cutoff on Lag poll strategy
...
### Fixes
* [#422] Workaround for observed bug where notification of reassignment on paused partitions may be lost

# [0.33.3]
atleon `0.33.3` is a beta release containing updates outlined below

### Fixes
* [#419] Fixed bug where revocation of partitions without any processed messages or revocation of empty partitions caused IllegalArgumentException

# [0.33.2]
atleon `0.33.2` is a beta release containing updates outlined below

### Additions
* [#416] Added explicit+configurable timeout on Kafka offset commits; Refactor encapsulation of async periodic commits

# [0.33.1]
atleon `0.33.1` is a beta release containing updates outlined below

### Fixes
* [#413] Updated Kafka Sender to propagate downstream context to upstream Publisher(s)

# [0.33.0]
atleon `0.33.0` is a beta release containing updates outlined below

### Additions
* [#396] Implemented optimized low-level reactive Kafka receiver and sender
* [#398] Make optimized Kafka clients usable by configuring `kafka.receiver.reception.type` and/or `kafka.sender.sending.type` to `"OPTIMIZED"`

# [0.32.1]
atleon `0.32.1` is a beta release containing updates outlined below

### Fixes
* [#407] Downgraded Kafka from 3.9.1 to 3.7.1
...
### Additions
# [#392] `Scheduler` can now be specified on `limitPerSecond` `GroupFlux` operators

# [0.31.5]
atleon `0.31.5` is a beta release containing updates outlined below

### Additions
# [#392] `Scheduler` can now be specified on `limitPerSecond` `AloFlux` operators

# [0.31.4]
atleon `0.31.4` is a beta release containing updates outlined below

### Fixes
* [#386] Deployment migrated from OSSRH to Maven Central
* [#387] Updated Kafka serializers to correctly handle `null` values
* [#389] Simplified and standardized `deduplication` APIs
...
### Fixes
* [#379] AWS-backed resources now support configuring `WebIdentityTokenFileCredentialsProvider` (thanks to @brandy-m)

# [0.31.1]
atleon `0.31.1` is a beta release containing updates outlined below

### Fixes
* [#370] `StreamPropertyResolver` interface reworked for consistency among similar project APIs (specifically, consistent usage of `Optional`)

# [0.31.0]
atleon `0.31.0` is a beta release containing updates outlined below

### Additions
* [#370] Added `SpringAloStream` to help bootstrap stream development in Spring with convenient property access
* [#372] `KafkaLagThresholdStarterStopper` updated to allow monitoring multiple consumer groups (thanks to @cmcquin)
* [#374] Overridable `autostart` configuration added to `AloStreamConfig` to configure auto-start/stop enablement 

### Fixes
* [#327] Instance IDs for component streams (of `CompositeAloStream`) now qualified with `i`

# [0.30.3]
atleon `0.30.3` is a beta release containing updates outlined below

### Fixes
* [#359] Prioritized Kafka client IDs now have priority suffix qualified with `p`, and are validated to be non-negative

# [0.30.2]
atleon `0.30.2` is a beta release containing updates outlined below

### Fixes
* [#366] `SingleMachinePartitionAssignor` updated to explicitly return assignments for all member IDs, even if empty

# [0.30.1]
atleon `0.30.1` is a beta release containing updates outlined below

### Additions
* [#357] Added `KafkaConfigSource::withConsumerPartitionAssignor` convenience method
...
### Fixes
* [#349] Bump versions of QPID, Curator, and ZooKeeper

# [0.28.3]
atleon `0.28.3` is a beta release containing updates outlined below

### Fixes
* [#334] Improve grace delay and add javadoc for Kafka Consumer mutex
