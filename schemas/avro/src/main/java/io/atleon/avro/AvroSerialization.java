...
public final class AvroSerialization {
    ...
    private static Schema generateWriterSchema(Object data, Type type, Function<Type, Schema> typeSchemaLoader) {
        return TypeResolution.isGenericClass(data.getClass()) && !TypeResolution.isDataStructure(data.getClass())
                ? Schema.createRecord(
                        ...
                        null,
                        false,
                        generateWriterSchemaFields(data, typeSchemaLoader))
    ...
    private static List<Schema.Field> generateWriterSchemaFields(Object data, Function<Type, Schema> typeSchemaLoader) {
        return FieldResolution.getAllFields(data.getClass()).stream()
                ...
                .map(field ->
                        generateWriterSchemaField(field, ValueResolution.getFieldValue(data, field), typeSchemaLoader))
    ...
    private static Schema.Field generateWriterSchemaField(
            Field field, Object value, Function<Type, Schema> typeSchemaLoader) {
        Schema fieldSchema = AvroSchemas.getOrSupply(
                value, () -> generateWriterSchema(value, field.getGenericType(), typeSchemaLoader));
