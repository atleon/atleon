...
public final class AloDecoratorConfig {
    ...
    private static <T, D extends AloDecorator<T>> Optional<List<AloDecorator<T>>> loadExplicit(
            Map<String, ?> properties, Class<D> superType) {
    ...
    private static <T> Optional<List<AloDecorator<T>>> instantiatePredefined(
            Map<String, ?> properties, Class<? extends AloDecorator<T>> superType, String typeName) {
