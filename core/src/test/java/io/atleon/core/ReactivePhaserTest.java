...
class ReactivePhaserTest {
    ...
    public void reactivePhasingBehavesIdenticallyToBlocking() {
        ...
        assertEquals(phaser.awaitAdvance(0), reactivePhaser.awaitAdvanceReactively(0).block());
        assertEquals(phaser.awaitAdvance(0), reactivePhaser.awaitAdvanceReactively(0).block());

        assertEquals(phaser.arrive(), reactivePhaser.arrive());

        assertEquals(phaser.awaitAdvance(0), reactivePhaser.awaitAdvanceReactively(0).block());
        assertEquals(phaser.awaitAdvance(1), reactivePhaser.awaitAdvanceReactively(1).block());
        ...
        assertEquals(phaser.arrive(), reactivePhaser.arrive());
        assertEquals(phaser.awaitAdvance(1), reactivePhaser.awaitAdvanceReactively(1).block());
        assertEquals(phaser.awaitAdvance(2), reactivePhaser.awaitAdvanceReactively(2).block());
        assertEquals(phaser.awaitAdvance(-1), reactivePhaser.awaitAdvanceReactively(-1).block());
    ...
    public void deadlockDoesNotOccurWhenPhaserIsInteractedWithDuringPublishing() {
        ...
        Integer arrivalPhase = reactivePhaser.arriveAndAwaitAdvanceReactively()
