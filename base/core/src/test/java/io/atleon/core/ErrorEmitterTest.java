...
class ErrorEmitterTest {
    ...
    @Test
    public void applyTo_givenUpstreamElements_expectsElementsEmitted() {
        ErrorEmitter<String> emitter = ErrorEmitter.create();
        Flux<String> upstream = Flux.just("a", "b", "c");

        StepVerifier.create(emitter.applyTo(upstream))
                .expectNext("a", "b", "c")
                .expectComplete()
    ...
    @Test
    public void applyTo_givenEmptyPublisher_expectsNoElements() {
        ErrorEmitter<String> emitter = ErrorEmitter.create();

        StepVerifier.create(emitter.applyTo(Flux.empty()))
                .expectSubscription()
                .expectComplete()
                .verify(Duration.ofSeconds(5));
    }
